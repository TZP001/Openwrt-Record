...
## AdGuardHome常用配置
### 安装（开始之前先把教程看完）
* 或许有更好的方法，dns转发国内组端口，监控组使用端口镜像（暂未实现）
#### 监控组（host模式）
```
docker run -dit \
  --name ADG_Eye \
...
#### DNS设置
##### 监控组
进去```<路由器ip>:3000```，把web监听端口设置为3000，DNS监听端口设置为5853（不要和其它端口冲突）<br>
###### 上游 DNS 服务器设置
```
127.0.0.1
```
...
DNS监听端口设置为6053（你映射的端口）,如果设置为53端口，会造成Adguardhome不工作，因为内部53端口已经被容器本身占用
###### 上游 DNS 服务器设置
```
